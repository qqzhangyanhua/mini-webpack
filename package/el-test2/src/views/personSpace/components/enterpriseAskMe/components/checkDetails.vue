<template>
  <el-dialog title="查看详情" :visible.sync="dialogVisible" width="50%" center>
    <div class="auth-check-detail">
      <h3>道路货物运输经营（含普通货运、专用运输、大件运输）许可-变更123</h3>
...
export default {
  ...
  methods: {
    getListItems() {
      if (this.compName === "applicationRecord") {
        const obj = {
          5: "申请撤销时间",
          2: "申请同意时间",
          4: "申请拒绝时间",
      ...
      if (this.itemStatus === STATUS_VALUE.APPLYING) {
        return [
          ...
          },
          {
            ...
            click: () => {
              applicationCanceled({ id: 1111 }).then(() => {
                this.dialogVisible = false;
                console.log("撤销申请");
              });
            },
